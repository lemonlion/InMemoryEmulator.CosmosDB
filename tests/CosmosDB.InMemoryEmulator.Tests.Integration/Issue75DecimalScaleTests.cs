...
///
/// Bug 2: SUM aggregate over decimal amounts returned "750.0" on Linux and "750" on Windows
...
public class Issue75DecimalScaleTests : IAsyncLifetime
{
	...
	/// <summary>
	/// Newtonsoft.Json serialises decimal 100.50m as "100.50". Real Cosmos DB's JavaScript
	/// engine normalises this to "100.5" on storage (JavaScript strips trailing zeros from
	/// fractional values). The in-memory emulator must match this behaviour.
	///
	/// Ref: JavaScript: JSON.stringify(JSON.parse("100.50")) === "100.5"
	/// </summary>
	[Trait(TestTraits.Target, TestTraits.InMemoryOnly)]
	[Fact]
	public async Task FractionalDecimal_RoundTrip_TrailingZeroIsStripped()
	{
		...
		// Ref: Real Cosmos DB (JavaScript engine) strips trailing zeros from fractional values.
		// "100.50" must round-trip as "100.5", not "100.50".
		readBack.Resource["amount"]!.ToString().Should().Be("100.5");
	}
